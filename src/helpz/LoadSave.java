...
public class LoadSave {
	...
	//Create a new lvl with default values
		...
		public static void CreateLevel(String name, int[] idArr) {
			...
			if(newLevel.exists()) {
				...
				WriteToFile(newLevel, idArr, new PathPoint(0,0), new PathPoint(0,0));  //PathPoint default pt start si end
			}
		}
	...
	public static ArrayList<PathPoint> GetLevelPathPoints(String name){
		File lvFile = new File("res/" + name + ".txt");
		
		if(lvFile.exists()) {
			ArrayList<Integer> list = ReadFromFile(lvFile);
	...
	public static int[][] GetLevelData(String name) {
		...
		if(lvFile.exists()) {
			ArrayList<Integer> list = ReadFromFile(lvFile);
			return Utilz.ArrayListT2Dint(list, 20, 20);
