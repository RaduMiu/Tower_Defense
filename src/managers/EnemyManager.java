...
public class EnemyManager {
	...
	private void loadEnemyImgs() {
		...
		for(int i=0; i<4; i++) {
			enemyImgs[i] = atlas.getSubimage(i*32, 32, 32, 32); //x, y, xsize, ysize
		}
	...
	private float getSpeedAndWidth(int dir, int enemyType) {
		if(dir == LEFT)
			return -GetSpeed(enemyType);  //left is negative on xOy axe
		else if(dir == RIGHT)
			return GetSpeed(enemyType) + 32;
	...
	private void drawHealthBar(Enemy e, Graphics g) {
		...
		g.fillRect((int)e.getX() + 16 -(getNewBarWidth(e) / 2), (int)e.getY()-10, getNewBarWidth(e), 3); //primul calcul face sa fie bara in jumate(16 jumatate din 32
								//(   ca sa fie in centru    )
	}
	
	private int getNewBarWidth(Enemy e) {  //modifica lungimea barei
