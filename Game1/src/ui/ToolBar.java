...
public class ToolBar extends Bar{
	...
	private void initPathImgs() { //buton S si E
	...
	public void rotateSprite() {
		...
		currentIndex++;
		if(currentIndex >= map.get(currentButton).size())  // pt a nu sari peste dimensiunea mapului
			currentIndex = 0;
		
		selectedTile = map.get(currentButton).get(currentIndex);
		editing.setSelectedTile(selectedTile);  //roteste figura selectata din dreptul mousului 
	}
	
	public void draws(Graphics g) {   //am pus draws sa nu incurc cu draw care este din clasa MyButton
		//Background
	...
	private void drawMapButtons(Graphics g) {
		...
		for(Map.Entry<MyButton, ArrayList<Tile>> entry : map.entrySet()) {
			MyButton b = entry.getKey();										// asa we get the button and the img
...
/*
	...
	*/
	...
	private void drawSelectedTile(Graphics g) {  //face casuta care indica ce tile e selectat
